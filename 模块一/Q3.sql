...
-- 1.2 增加条件：平均成绩大于70
SELECT s.id, s.name, AVG(sc.score) AS avg
...
SELECT s.id, s.name, COUNT(c.id) as num_course, SUM(sc.score) as total_score
FROM student_course sc 
JOIN student s on (s.id = sc.student_id) 
JOIN course c on (c.id = sc.course_id)
GROUP BY s.id, s.name
...
SELECT s.id, s.name, COUNT(c.id) as num_course
FROM student_course sc 
JOIN student s on (s.id = sc.student_id) 
JOIN course c on (c.id = sc.course_id)
GROUP BY s.id, s.name
